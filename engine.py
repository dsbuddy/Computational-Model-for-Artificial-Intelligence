...
# def reset()
# 	global SIGMA, DELTA, Q, EPSILON, trialsRun, inputFile, usingFile, m, EpislonCanLearn


def main(setup, inputFile, outputFile, experiment):
	global SIGMA,DELTA,m,Q,exp
	exp = experiment
	LoadFromFile(setup)
	m = Model(SIGMA,DELTA)
	m.inputFileName = inputFile
...
class Model(object):
	...
	def __init__(self,Sigma,Delta,tau=1,alpha=0.05,beta=0.05,gamma=0.2,eta=1.0,zeta=0.0,nu=0.0,kappa=0.0):
		...
		self.outputFile = 'secondOrderOutput.txt'
		self.inputFileName = 'secondOrderConditioningInput.txt'
...
class Transition(object):
	...
	def getTransition(self):
		return (self.startState.id, self.endState.id)
...
def GetInput():
	...
	if not usingFile:
		Iin = []
		userInput = input('\nType \'run\' or enter symbol strength pairs seperated by , :\n')
		...
		# if runIt:
			usingFile = True
			return GetFileInput()
		if userInput.lower() == 'erase':
			os.remove(m.outputFile)
...
def GetFileInput():
	global inputFile,usingFile
	if inputFile == None:
		inputFile = open(m.inputFileName,'r')
	Iin = []
...
def SaveStatusToFile(status):
	global m, trialsRun, outSym, exp

	if not os.path.exists(m.outputFile):
		trialsRun = 0
		f = open(m.outputFile,"w+")
	trialsRun += 1

	file = open(m.outputFile,'a+')
	file.write("\n\n------------------------- Trial #" + str(trialsRun) + " -------------------------\n\n")
	file.write(status)
...
def checkOutSym(t, a):
	global exp, trialsRun, outSym

	if exp == 1:
		if t.getTransition()[0] == 0:
			if (str(t.getTransition()[1]) == "3"):
				# print(str(a))
